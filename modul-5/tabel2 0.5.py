...
def load_data_from_excel(excel_path="modul-5/modul5.xlsx"):
    ...
    """
    df_dict = {}
    sheet_names = ["kertas", "Mika", "Stik", "4_kertas_vertikal"] # Case sensitive as in notebook
...
# --- Add new function to compute modulus of resilience using integration ---
def compute_resilience_integral(strain, stress, epsilon_y):
    """
    Computes modulus of resilience by integrating the stress-strain curve
    up to the yield strain (epsilon_y).
    """
    if strain is None or stress is None or len(strain) != len(stress):
        return np.nan

    valid_mask = (strain >= 0) & (strain <= epsilon_y)
    if np.sum(valid_mask) < 2:
        return np.nan

    return simpson(stress[valid_mask], strain[valid_mask])
...
def calculate_mechanical_properties(dataframe, L0_mm, A0_m2, material_type=None, initial_points_config=None):
    ...
    # New integral-based method
    Ur_integral_J_m3 = compute_resilience_integral(strain, stress_Pa, epsilon_y) if epsilon_y and epsilon_y > 0 else np.nan
    ...
    return {
        ...
        'epsilon_y': epsilon_y, 
        'Ur_J_m3': Ur_J_m3,
        'Ur_integral_J_m3': Ur_integral_J_m3
    }
...
if data_frames:
    material_variations = {
        ...
        "4_kertas_vertikal": ["4_kertas_vertikal_vertikal_1"],
    }

    for var_name, sample_keys in material_variations.items():
        material_type = None # Will be "kertas", "Mika", or "Stik"
        if "4_kertas_vertikal" in var_name.lower():
            material_type = "4_kertas_vertikal"
        elif "kertas" in var_name.lower(): # Use lower() for robust matching
        ...
        for sample_key in sample_keys:
            if sample_key in data_frames:
                ...
                u_resiliences_j_m3_integral.append(props['Ur_integral_J_m3'])
                
                print(f"  Sample {sample_key}: E={props['E_Pa']:.2e} Pa, σ_y={props['sigma_y_Pa']:.2e} Pa, ε_y={props['epsilon_y']:.4f}, U_r={props['Ur_J_m3']:.2e} J/m³, U_r_int={props['Ur_integral_J_m3']:.2e} J/m³")
            else:
                ...
                u_resiliences_j_m3_integral.append(np.nan)
        ...
        avg_Ur_J_m3_integral = np.nanmean(u_resiliences_j_m3_integral) if len(u_resiliences_j_m3_integral) > 0 else np.nan
        ...
        results_summary[var_name] = {
            ...
            'resilience_modulus_integral_J_m3_avg': avg_Ur_J_m3_integral,
...
if __name__ == "__main__":
    ...
    if not data_frames:
        ...
        for var, props in results_summary.items():
            ...
            rm_int_str = f"{props['resilience_modulus_integral_J_m3_avg']:.3e}" if not np.isnan(props['resilience_modulus_integral_J_m3_avg']) else "N/A"
            ...
            print(f"  Average Resilience Modulus (Integral J/m^3): {rm_int_str}")
            if props['notes']:
